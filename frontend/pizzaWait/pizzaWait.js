  ...
  console.log(userActiveAcceptance);
    ...
    nums.forEach((num, idx) => {
      ...
      num.addEventListener('animationend', (e) => {
        if (e.animationName === 'goIn' && idx !== penultimate) {
          ...
          if (idx === penultimate) {
            // This is the last span (0)
            num.nextElementSibling.classList.add('in');
            num.nextElementSibling.addEventListener('animationend', () => {
            });
          } else {
            num.nextElementSibling.classList.add('in');
          }
  ...
  async function startTaskCall() {
    try {
        const url = new URL("http://localhost:8080/users/startTasks");
        const params = new URLSearchParams();
        params.append("email", parsedResponse.email);
        ...
        user_params.append("email", parsedResponse.email);
        ...
        // Store the response data in localStorage if needed
